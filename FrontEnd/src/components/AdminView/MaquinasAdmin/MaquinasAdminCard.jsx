...
const MaquinasAdminCard = ({ maquina, filtro, abrirModal }) => {
    ...
    switch (filtro) {
        ...
        case "Sin Reponer":
            ...
            return (
                <article key={maquina.id_maquina} className='flex flex-col items-center justify-center border rounded-xl text-center py-8 h-full cursor-pointer hover:shadow-lg hover:shadow-yellow-950'>
                    <span className='font-bold text-xl'>{maquina.id_maquina}</span>
                    <p>{maquina.nombre_establecimiento}</p>
